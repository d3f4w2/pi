...
pigo --session <path|id> # Use a specific session file or partial session ID
pigo --fork <path|id>    # Fork a session file or partial session ID into a new session
```
...
`/resume` opens an interactive session picker for the current project. `pigo -r` opens the same picker at startup.
...
```bash
pigo --name "Refactor auth module"
pigo --name "CI audit" -p "Review this build failure"
```

Named sessions are easier to find in `/resume` and `pigo -r`.
