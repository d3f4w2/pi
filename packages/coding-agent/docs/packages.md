...
```bash
pigo install npm:@foo/bar@1.0.0
pigo install git:github.com/user/repo@v1
pigo install https://github.com/user/repo  # raw URLs work too
pigo install /absolute/path/to/package
pigo install ./relative/path/to/package

pigo remove npm:@foo/bar
pigo list                     # show installed packages from settings
pigo update                   # update pi only
pigo update --all             # update pi, update packages, and reconcile pinned git refs
pigo update --extensions      # update packages and reconcile pinned git refs only
...
pigo update npm:@foo/bar      # update one package
pigo update --extension npm:@foo/bar
```

These commands manage pi packages and `pigo update` can update the pi CLI installation. To uninstall pi itself, see [Quickstart](quickstart.md#uninstall).
...
```bash
pigo -e npm:@foo/bar
pigo -e git:github.com/user/repo
```

## Package Sources

Pi accepts three source types in settings and `pigo install`.
...
- Versioned specs are pinned and skipped by package updates (`pigo update --extensions`, `pigo update --all`).
...
- Refs are pinned tags or commits. `pigo update --extensions` and `pigo update --all` do not move them to newer refs, but they do reconcile an existing clone to the configured ref.
- Use `pigo install git:host/user/repo@new-ref` to update settings and move an existing package to a new pinned ref.
...
# With version ref
pigo install git:git@github.com:user/repo@v1.0.0
```
...
Use `pigo config` to enable or disable extensions, skills, prompt templates, and themes from installed packages and local directories. `pigo config` starts in global settings (`~/.pi/agent/settings.json`); press Tab to switch between global and project-local modes. Use `pigo config -l` to start in project overrides (`.pi/settings.json`) with inherited global resources dimmed.
