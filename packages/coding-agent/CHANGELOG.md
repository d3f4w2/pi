...
- Added bounded isolated `task` workers for concurrent research and coding in temporary project snapshots, with role-specific tools, a three-worker limit, hard timeouts, bounded results, parent review, and no automatic write merging.
- Added append-only context checkpoints and rewinds with evidence-preserving previews; an eleven-language AST registry; a project-local shared LSP broker with one-shot private fallback; and a bounded read-only Eval bridge for Python and Bun cells.
- Added a unified external-information plane with official-source routing, typed resource addresses, metadata-rich caching, concurrent fill coalescing, and Browser 2.0 tabs, waits, versioned references, protected uploads/downloads, diagnostics, and screenshots.
- Added MCP OAuth 2.1 authorization with discovery, dynamic registration, PKCE, loopback callbacks, issuer/state checks, and credential-isolated storage; and ACP routing through client-declared filesystem and terminal capabilities without duplicate local retries.
- Added a repeatable Windows source-launch benchmark with interleaved `tsx` and Node strip-only runs, median and P95 results, machine and revision metadata, raw samples, and a documented capability baseline.
- Added MCP stdio, Streamable HTTP, and SSE integration with lazy cached discovery, local approval mapping, resources and prompts; an ACP editor mode backed by the existing Agent session; and controlled plugins with per-file integrity, capability manifests, explicit enablement, lifecycle-script blocking, atomic update recovery, and rollback.
- Added a fail-closed sandbox for built-in file and process tools, with canonical path guards, secret-free environments, Bubblewrap/Seatbelt enforcement on Unix, automatic separate-user/WFP isolation after one Windows setup command, integrity-checked per-process Windows helper materialization, exact-destination on-demand network approval with command/session/workspace lifetimes, and a zero-configuration restricted-token/Job Object fallback with explicit documented read/network limits.
- Expanded the structured `git` tool with read-only atomic commit-plan validation, dependency-cycle rejection, bounded base/ours/theirs conflict inspection, stale-safe whole-file or edited-worktree resolution, exact staging, and rollback when conflict staging fails.
- Unified the built-in read tool across anchored local text, images, SSRF-protected webpages, page-selectable PDFs, bounded ZIP/TAR/TAR.GZ archives, file and pi URIs, and pluggable internal resource resolvers with shared truncation and source metadata.
- Expanded the DAP debugger to 23 operations with real launch and process/port attach flows, thread listing and pause, conditional, hit-count, log, function, exception, and data breakpoints, source/module inspection, restart, capability negotiation, and separate disconnect/terminate semantics for Python, JavaScript/TypeScript, and Go.
- Added a Windows-native execution foundation: built-in grep/find use managed rg/fd when available with a concurrent filesystem fallback, require no manual rg, fd, Git Bash, or WSL installation, and the PowerShell executor now launches directly instead of being nested inside Bash.
- Expanded the built-in LSP tool to fifteen operations with type definitions, safe file rename and reference updates, preview-first code actions, server status and reload, capability inspection, and approval-gated raw requests.
- Added a controlled self-evolution loop that clusters privacy-safe repeated failures, creates one user-reviewed prompt or triggered-strategy candidate, runs paired isolated Agent A/B evidence, binds evaluation and approval to an immutable digest, canaries matching real tasks, automatically stops failed canaries, and supports explicit promotion, pause, history, and rollback through `/learn`.
- Added an evidence-learning memory loop with always-available structured remember, propose, recall, feedback, and forget operations; user-approved direct writes; one Agent candidate per task with immediate review; typed temporal claims; movable file-excerpt evidence; bounded core and project recall; usage outcomes; prototype-store migration; secret and authority protections; and a compact `/memory` current, pending, and history interface.
- Added real Agent capability evaluation through `/evals`, launching fresh RPC Agent processes in isolated temporary fixtures with one initial case each for navigation, bug fixing, verification, recovery, scope control, and durable memory, hidden post-run verification, required-tool checks, two-minute case timeouts, token budgets, explicit cost confirmation, and compact local results.
...
- Reduced startup and tool latency with a bootstrap-only version path, split Node CLI bundle, lazy mode and optional-parser loading, concurrent deterministic extension imports, overlapping model/resource initialization, reusable model-host preconnects, native Windows rg/fd execution, and ordered shared/exclusive tool scheduling.
