...
describe("theme syntax highlighting", () => {
	...
	it("colors diff additions and deletions in fenced diff blocks", () => {
		...
		expect(lines[0]).toBe("\x1b[38;2;231;130;132m-old\x1b[39m");
		expect(lines[1]).toBe("\x1b[38;2;159;207;159m+new\x1b[39m");
	});
	...
	it("keeps cli-highlight default styled scopes mapped to theme styles", () => {
		expect(highlightCode("const re = /foo+/gi;", "javascript")[0]).toContain(
			"\x1b[38;2;168;199;160m/foo+/gi\x1b[39m",
		);
		expect(highlightCode("@decorator", "python")[0]).toBe("\x1b[38;2;164;173;184m@decorator\x1b[39m");
		expect(highlightCode("<div></div>", "html")[0]).toContain("\x1b[38;2;125;211;252mdiv\x1b[39m");
	});
});
